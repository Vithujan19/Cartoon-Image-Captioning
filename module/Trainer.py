...
class Trainer:
    ...
    def predict_caption(
            self,
            image_path,
            beam_search_pred=True,
            load_from_checkpoint=True,
    ):
        sample_img_tensor_val = load_image_features_on_the_fly(image_path)
        ...
        if beam_search_pred:
            prediction = self.beam_search_pred(sample_img_tensor_val)
        else:
            prediction, _ = self.greedy_search_pred(sample_img_tensor_val, compute_attention_plot=False)
        return prediction
    ...
    def initiate_training(
        ...
        for epoch in range(start_epoch, EPOCHS + 1):
            ...
            if avg_train_loss_this_epoch < min_train_loss:
                print(
                    f'this epoch has least loss, saving a checkpoint, epoch = {epoch}, loss = {avg_train_loss_this_epoch}')
            ...
            print(
                f'Epoch {epoch} Average Train Loss {avg_train_loss_this_epoch:.6f}, Average Val Loss {avg_val_loss_this_epoch:.6f}, Time taken {time.time() - start:.2f}sec\n')
    ...
    def compute_bleu_scores(self, config, group='val', search_method='greedy', filter_files=[], save_to_dir=True):
        ...
        df['caption'] = df['caption'].apply(
            lambda x: ' '.join(x.split()[1:-1]))  # remove startseq & endseq from captions
        ...
        for filename, captions_list in filename_to_captions_dict.items():
            ...
            bleu_3 = round(sentence_bleu(captions_list_words, predicted_caption_words, weights=[0.33, 0.33, 0.33, 0]),
                           3)
            bleu_4 = round(
                sentence_bleu(captions_list_words, predicted_caption_words, weights=[0.25, 0.25, 0.25, 0.25]), 3)
        ...
        # write to disk
        if save_to_dir:
            filename = config['store'] + config['nn_params']['pred_df_dir'] + 'result_{}_{}.csv'.format(group,
                                                                                                        search_method)
